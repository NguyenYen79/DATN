...
def generate_frames(quality=85, scale=1.0, fps_limit=15):
    """
    quality  : JPEG quality 1-95
    scale    : resize trước encode (0.5 = half res, nhanh gấp đôi)
    fps_limit: giới hạn FPS tránh spam bandwidth
    """
    global people_count
    ...
    if HAILO_AVAILABLE and hailo_model is not None:
        print(f"[STREAM] Hailo | quality={quality} scale={scale} fps={fps_limit}")
                ...
                while True:
                    ...
                    if ai_mode:
                        try:
                            ...
                            print(f"[HAILO INFER] {e}")
                    data = maybe_yield(frame)
                    if data: yield data
    else:
        print(f"[STREAM] CPU | quality={quality} scale={scale} fps={fps_limit}")
        while True:
            ok, frame = camera.read()
            if not ok: break
            data = maybe_yield(frame)
...
def video_feed():
    if not session.get('logged_in'):
        return redirect(url_for('login'))
    # Nhận params từ frontend: ?quality=60&scale=0.6&fps=10
    quality   = int(request.args.get('quality', 85))
    scale     = float(request.args.get('scale',   1.0))
    fps_limit = int(request.args.get('fps',       15))
    return Response(
        generate_frames(quality=quality, scale=scale, fps_limit=fps_limit),
